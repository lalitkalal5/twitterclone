...
//     try {
//       await axios.post('https://serverfortwitterclone-3.onrender.com/tweet', { owner, content });
//       setContent('');
//       // Refresh tweets
//       const response = await axios.post('https://serverfortwitterclone-3.onrender.com/gettweet', { owner });
//       setTweet(response.data.reverse());
...
//               <div
//                 key={index}
//                 className="border-b border-gray-200 py-4 hover:bg-gray-50 transition rounded-lg px-2"
//               >
//                 <Link to={`/userprofile/${t.owner}`} className="text-sm text-blue-500 hover:underline">
//                   @{t.owner}
//                 </Link>
...
//             ))
//           ) : (
//             <p className="text-center text-gray-500 mt-6">No tweets yet. Start posting!</p>
//           )} */}
//           {/* Tweets List */}
// {loading ? (
//   // Skeleton loader
...
//       {/* Profile Picture */}
//       <img
//         src={t.owner.profilePic || "https://via.placeholder.com/40"}
//         alt="profile"
...
const Feed2 = () => {
  ...
  const handleNewTweetSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "https://serverfortwitterclone-3.onrender.com/tweet",
        { owner, content }
      );
      setContent("");
      const response = await axios.post(
      ...
      <div className="flex max-w-7xl mx-auto mt-4 px-4 gap-4">
        ...
        <main className="flex-1 bg-white rounded-xl shadow-sm p-4">
          ...
          {loading ? (
            ...
            tweet.map((t, index) => {
              // handle both populated (object) and old (string) owner
              const ownerObj =
                t && t.owner && typeof t.owner === "object" ? t.owner : null;
              const ownerId = ownerObj?._id || t.owner; // if string, still linkable
              const username = ownerObj?.username || "Unknown";
              const avatar = ownerObj?.profilePic || "https://via.placeholder.com/40";

              const date =
                t.createdAt
                  ? new Date(t.createdAt)
                  : objectIdToDate(t._id); // fallback for old docs
